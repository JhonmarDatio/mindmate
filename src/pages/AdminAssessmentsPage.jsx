...
const AdminAssessmentsPage = () => {
  ...
  useEffect(() => {
    const fetchAssessments = async () => {
      ...
      if (!error) {
        // student_name comes from the profiles join in databaseUtils
        setAssessments(data || [])
      }
      setLoading(false)
    }
  ...
  const filteredAssessments = assessments.filter((a) => {
    ...
    return (
      !searchTerm ||
      name.includes(q) ||
      (a.stress_level || '').toLowerCase().includes(q)
    )
  })
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                filteredAssessments.map((assessment, index) => {
                  ...
                  const studentName = consented
                    ? (assessment.student_name || 'Unknown Student')
                    : 'Anonymous'
                    ...
                    >
                      <td className="px-6 py-3.5 text-sm font-semibold">
                        <span className={`group-hover:text-emerald-600 transition-colors ${
                          !consented ? 'text-gray-400 italic font-medium' : 'text-gray-700 font-semibold'
                        }`}>
                          {studentName}
                        </span>
                      </td>
            ...
            <div className="grid grid-cols-2 gap-y-4 mb-5 text-sm">
              <div>
                ...
                <p className={`font-semibold mt-0.5 ${!selectedAssessment.consent_status ? 'text-gray-400 italic' : 'text-gray-800'}`}>
                  {selectedAssessment.consent_status
                    ? (selectedAssessment.student_name || 'Unknown Student')
                    : 'Anonymous'}
                </p>
              </div>
              <div>
                ...
                <p className="font-medium text-gray-700 mt-0.5">
                  {new Date(selectedAssessment.created_at).toLocaleDateString('en-US', {
                    month: 'short', day: 'numeric', year: 'numeric',
                  })} • {new Date(selectedAssessment.created_at).toLocaleTimeString([], {
